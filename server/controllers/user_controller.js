...
const signUp = async (req, res) => {
  ...
  const checkBirthday = (birthday) => {
    ...
    const result =
      birthdayNumbers[0] == year &&
      birthdayNumbers[1] == month &&
      birthdayNumbers[2] == date &&
      year >= 1950 &&
      year <= 2020;
    return result;
  };
  ...
  if (!checkBirthday(birthday)) {
    res
      .status(400)
      .send({
        error: "Please enter a date between 1950-01-01 and 2020-12-31.",
      });
    return;
  }
  ...
  if (!name || !email || !password || !birthday) {
    res
      ...
      .send({
        error:
          "Request Error: name, email, birthday and password are required.",
      });
    return;
  }
